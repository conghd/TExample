...
void Matrix::vec3_multiply_mat3( vec3 *dst, vec3 *v, mat3 *m )
{
	dst->x = ( v->x * m->m[ 0 ].x ) +
			 ( v->y * m->m[ 1 ].x ) +
			 ( v->z * m->m[ 2 ].x );
...
void Matrix::vec3_multiply_mat4( vec3 *dst, vec3 *v, mat4 *m )
{
	dst->x = ( v->x * m->m[ 0 ].x ) +
			 ( v->y * m->m[ 1 ].x ) +
			 ( v->z * m->m[ 2 ].x );
...
void Matrix::vec4_multiply_mat4( vec4 *dst, vec4 *v, mat4 *m )
{
	dst->x = ( v->x * m->m[ 0 ].x ) +
			 ( v->y * m->m[ 1 ].x ) +
			 ( v->z * m->m[ 2 ].x ) +
...
void Matrix::mat3_identity( mat3 *m )
{
...
void Matrix::mat3_copy_mat4( mat3 *dst, mat4 *m )
{
...
void Matrix::mat4_identity( mat4 *m )
{
...
void Matrix::mat4_copy_mat4( mat4 *dst, mat4 *m )
{ memcpy( dst, m, sizeof( mat4 ) ); }


void Matrix::mat4_translate( mat4 *dst, mat4 *m, vec3 *v )
{
...
void Matrix::mat4_rotate_fast( mat4 *m, vec4 *v )
{
...
void Matrix::mat4_rotate( mat4 *dst, mat4 *m, vec4 *v )
{
	float s = sinf( v->w * DEG_TO_RAD ),
		  c = cosf( v->w * DEG_TO_RAD ),
		  xx,
	...
	if( !v->w || !Vector::vec3_normalize( &t, &t ) ) return;
...
void Matrix::mat4_scale( mat4 *dst, mat4 *m, vec3 *v )
{
...
unsigned char Matrix::mat4_invert( mat4 *m )
{
	mat4 mat;
...
unsigned char Matrix::mat4_invert_full( mat4 *m )
{
	mat4 inv;
...
void Matrix::mat4_transpose( mat4 *m )
{
	float t;
...
void Matrix::mat4_ortho( mat4 *dst, float left, float right, float bottom, float top, float clip_start, float clip_end )
{
	mat4 mat;
...
void Matrix::mat4_copy_mat3( mat4 *dst, mat3 *m )
{
	memcpy( &dst->m[ 0 ], &m->m[ 0 ], sizeof( vec3 ) );
	memcpy( &dst->m[ 1 ], &m->m[ 1 ], sizeof( vec3 ) );
	memcpy( &dst->m[ 2 ], &m->m[ 2 ], sizeof( vec3 ) );
...
void Matrix::mat4_multiply_mat3( mat4 *dst, mat4 *m0, mat3 *m1 )
{
	mat3 mat;
...
void Matrix::mat4_multiply_mat4( mat4 *dst, mat4 *m0, mat4 *m1 )
{
	mat4 mat;
